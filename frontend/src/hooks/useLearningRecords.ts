// ============================================================
// 【このファイル全体の方針】
// 【面接で説明できるようにする】なぜ useQuery を使うか
//   → コンポーネントがマウントされたタイミングで自動で API を叩き、
//     isLoading / isError / data を自動で管理してくれるから。
//     useState + useEffect で手書きするより責務が明確になる。
// 【面接で説明できるようにする】なぜ queryKey に criteria（検索条件）を含めるか
//   → criteria の中身が変わると queryKey が変わり、対応するキャッシュがなくなるので
//     自動で API を叩き直す。検索条件が変わるたびに再フェッチが走る仕組みをこれだけで実現できる。
// 【AI任せでOK】useQuery の型引数 <LearningRecord[]> の書き方
// ============================================================
